...
export function PatientClinicalDashboard({ patient, vitals }: PatientClinicalDashboardProps) {
  const [consultations, setConsultations] = useState([{ name: "Heart Burn", type: "", details: "" }, { name: "Fever", type: "", details: "" }]);
  const [diagnoses, setDiagnoses] = useState([{ name: "Heart Burn", type: "", details: "" }, { name: "Fever", type: "", details: "" }]);
  const [medications, setMedications] = useState([{ name: "Pepcid AC", frequency: "", duration: "", instruction: "" }]);
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-6 space-y-6 bg-slate-50/50 min-h-screen">
      {/* Top Section */}
      ...
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="past-records" className="border-none shadow-sm bg-white rounded-xl overflow-hidden px-6">
      ...
      <div className="space-y-6">
        <ClinicalSection 
          ...
          type="consultation"
          items={consultations}
          onAdd={() => setConsultations([...consultations, { name: "", type: "", details: "" }])}
          onRemove={(idx) => setConsultations(consultations.filter((_, i) => i !== idx))}
        ...
        <ClinicalSection 
          ...
          type="diagnosis"
          items={diagnoses}
          onAdd={() => setDiagnoses([...diagnoses, { name: "", type: "", details: "" }])}
          onRemove={(idx) => setDiagnoses(diagnoses.filter((_, i) => i !== idx))}
          onUpdate={(idx, field, val) => {
        ...
        <ClinicalSection 
          ...
          type="medication"
          items={medications}
          onAdd={() => setMedications([...medications, { name: "", frequency: "", duration: "", instruction: "" }])}
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {followUpDate ? format(followUpDate, "PPP") : <span>Follow Up Date</span>}
                </Button>
              </PopoverTrigger>
