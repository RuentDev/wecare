        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              Managing <span className="font-semibold text-primary">{user.first_name} {user.last_name}</span>
              <Badge variant="outline" className="ml-2 font-mono text-[10px]">ID: {user.id.substring(0, 8)}</Badge>
            </p>
          </div>
        </div>
        <Badge className={user.is_active ? "bg-success text-white" : "bg-destructive text-white"}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="glassmorphism-card border-none overflow-hidden rounded-[24px]">
            <CardHeader className="bg-linear-to-r from-primary/5 to-transparent border-b border-white/20">
              <CardTitle className="flex items-center gap-2 text-xl">
                <UserCog className="w-5 h-5 text-primary" /> Edit User Information
              </CardTitle>
            </CardHeader>
        ...
        <div className="space-y-6">
          <Card className="glassmorphism border-none rounded-[24px]">
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col gap-1">
                ...
                <Badge variant="secondary" className="w-fit capitalize bg-primary/10 text-primary border-primary/20">
                  {user.role}
                </Badge>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-xs text-muted-foreground font-medium">RBAC Permissions</span>
                <div className="flex flex-wrap gap-1">
                  {user.user_roles?.map((ur: any) => (
                    <Badge key={ur.roles.id} variant="outline" className="text-[10px]">
                      {ur.roles.name}
                    </Badge>
                  ))}
          ...
          <Card className="glassmorphism border-none rounded-[24px] bg-amber-50/10 border-amber-200/20">
            <CardHeader>
              <CardTitle className="text-lg text-amber-700 dark:text-amber-400">Danger Zone</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
